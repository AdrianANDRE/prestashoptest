...
Test Setup
	...
	${__TEST_15_SETUP}	Get Variable Value	${TEST 15 SETUP}
	Run Keyword If	$__TEST_SETUP is not None	${__TEST_SETUP}
	Run Keyword If	$__TEST_15_SETUP is not None	${__TEST_15_SETUP}

Test Teardown
	${__TEST_15_TEARDOWN}	Get Variable Value	${TEST 15 TEARDOWN}
	${__TEST_TEARDOWN}	Get Variable Value	${TEST TEARDOWN}
	Run Keyword If	$__TEST_15_TEARDOWN is not None	${__TEST_15_TEARDOWN}
	Run Keyword If	$__TEST_TEARDOWN is not None	${__TEST_TEARDOWN}

*** Test Cases ***
Place an order with 2 article , log on user : "user" $$ test
